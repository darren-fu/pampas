...
public class ServerInstance {
     ...
     * 服务分组
     * 同一个service下可能有多组服务，
     * 比如 注册多个Interface到同一个serviceName下
     */
    ...
    public ServerInstance ready() {
        ...
        this.setStartTimestamp(ObjectUtils.defaultIfNull(this.startTimestamp,
                Long.valueOf(getPropValue(PampasConsts.Instance.PROP_TIMESTAMP_KEY, PampasConsts.Instance.PROP_TIMESTAMP_DEFAULT))));
        //权重
        this.setWeight(ObjectUtils.defaultIfNull(this.weight,
                Integer.valueOf(getPropValue(PampasConsts.Instance.PROP_WEIGHT_KEY, PampasConsts.Instance.PROP_WEIGHT_DEFAULT))));

        //设置预热时间
        this.setWarmupSeconds(ObjectUtils.defaultIfNull(this.warmupSeconds,
                Integer.valueOf(getPropValue(PampasConsts.Instance.PROP_WARMUP_KEY, PampasConsts.Instance.PROP_WARMUP_DEFAULT))));
